...
中文智能体（AI Agent）安全研究资料合集，共七份配套文档，从攻防全景到落地评估手册、专项测试与全执行链取证形成完整链路。
...
| [05. Agent 沙箱逃逸专题研究](./05-Agent-Sandbox-Escape-Research.md) | 系统层逃逸专项：procfs 逃逸全家桶（/proc/pid/root 等）、Bubblewrap CVE 家谱（2017–2026）、三梯队横向测试嫌疑名单、30 分钟快速探针包与深度测试方案 |
| [06. 智能体目标劫持横向测试指南](./06-Agent-Goal-Hijacking-Test-Guide.md) | ASI03 专项：从微软 EchoLeak（CVE-2025-32711）提炼的五步攻击链通用模型、L1/L2/L3 三级测试用例库（目标替换/越权动作/外传观测）、金丝雀+自控接收端的安全测试方法、ASR×Utility 评分表与报告模板 |
| [07. Agent 全执行链观察与取证指南](./07-Agent-Execution-Trace-Observability.md) | 面向大模型安全研究人员的 Agent 工作循环、统一 Trace/Span 模型、现有分析工具矩阵、编码 Agent 黑匣子架构、MVP、评价指标与研究伦理 |
